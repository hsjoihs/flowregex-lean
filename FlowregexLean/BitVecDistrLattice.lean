...
theorem List.nodup_map_id_prod {T : Type} {f : Nat → T} {l : List Nat} :
  l.Nodup -> (List.map (fun k => (k, f k)) l : List (Nat × T)).Nodup := by
  unfold List.Nodup
  ...
  | cons x xs ih =>
    ...
    simp only [List.pairwise_cons] at h_nodup
    constructor
    · have h2 := ih h_nodup.right
      intro a
      intro h_a_xs
      have h3 := h_nodup.left a h_a_xs
      contrapose! h3
      simp at h3
      exact h3.left
    · have h2 := ih h_nodup.right
